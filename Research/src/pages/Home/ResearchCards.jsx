...
const ResearchCards = () => {
  ...
  const renderCard = (plan, index) => {
    ...
    return (
      <div key={index} className="flex flex-col card-shine w-full max-w-[300px] min-h-[640px] h-full">
        <div
          className={`${plan.bgColor} ${plan.textColor} ${plan.borderColor} flex flex-col h-full p-6 rounded-t-2xl`}
        ...
        <Slider {...mobileSliderSettings}>
          {plans.map((plan, index) => (
            <div key={index} className="px-2">
              <div className="mx-auto">{renderCard(plan, index)}</div>
            </div>
          ))}
        </Slider>
      </div>
