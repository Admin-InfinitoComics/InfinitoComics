...
<div className="flex flex-col md:flex-row justify-between items-center p-5 rounded mb-8 mx-4 sm:mx-10 md:mx-20">
        ...
        <div className="w-full">
          ...
          {[...new Set(filteredJobs.map(job => job.department))].map(dept => (
<div key={dept} className="mb-10 mx-4 sm:mx-10 md:mx-20 lg:mx-40">
              ...
              <div className="flex flex-col gap-4 mx-2 my-16">
                {filteredJobs
                  ...
                  .map(job => (
                   <div key={job.id} className="flex flex-col sm:flex-row gap-2 sm:gap-0 justify-between items-start sm:items-center px-4 md:px-10 my-2">

                      {/* Job Title */}
                   <div className="text-gray-700 font-medium w-full sm:w-64 truncate">{job.title}</div>
<div className="text-sm text-gray-500 w-full sm:w-40 text-left sm:text-center whitespace-nowrap">{job.positions} positions</div>
<div className="w-full sm:w-32 text-left sm:text-right">
  <a href="/#" className="text-blue-600 tracking-widest">Apply {">"}</a>
</div>
