...
const getNextEventNumber = () => {
  ...
  const handleCreateTimeline = async (blockId, index) => {
    ...
    try {
      ...
      const result = await addItems({
        ...
        category,
        description,
        image: images[0], // pick first image only
  ...
  // Set timelineBlocks to start with the next event number
  setTimelineBlocks([
    {
      id: Date.now(),
...
setCurrentPage(1);
 ...
 const handleEdit = (item, isUpdate = false) => {
  ...
  const newBlock = {
    ...
    date: item.eventDate ? item.eventDate.split('T')[0] : '', // fallback
    images: [item.imageUrl],
    isEditable: isUpdate
  };

  // Add and sort by eventNumber
...
const updateEventNumbersInDB = async (blocks) => {
  for (const block of blocks) {
    await updateItems(block._id, {
      title: block.title || "",
      eventDate: block.date || "", // Make sure this is a valid date string
      category: block.category || "",
      description: block.description || "",
      eventNumber: block.eventNumber, // This is the key field!
      image: block.images?.[0] || "",
      imageUrl: typeof block.images?.[0] === 'string' ? block.images[0] : undefined
    });
  }
                    ...
                    const isOddEvent = Number(block.eventNumber) % 2 === 1;
                      ...
                      return (
                        <div key={block.id} className="grid md:grid-cols-[1fr_auto_1fr] min-h-[350px] items-stretch gap-4  sm:grid-cols-1 sm:min-h-[250px] sm:p-2 p-4 lg:gap-0 lg:p-0">
                          {isOddEvent ? (
                            // ODD: Text left, RED line, Image right
                            <>
                              {/* Text Left */}
                          ...
                          >
                            ...
                            // EVEN: Image left, GRAY line, Text right
                            <>
                              {/* Image Left */}
                              <div className="flex flex-col justify-center items-center">
                                {block.images.length > 0 ? (
